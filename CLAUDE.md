...
domain/                l'app Django del modello di dominio v1
  ...
  publication.py        la **pubblicazione** verso la griglia piatta di Aurora
                        (ADR-027 §3.2, ADR-033): 142 chiavi su 142 se la
                        maschera si legge, e la `Perdita` conta ciò che il
                        confine non tiene — la parte che esce come la sua
                        classe, il sabato che Aurora non ha, la cella ambigua
                        che si **nomina** invece di risolversi
...
> minimo. **1081 test**, 17 skip tutti misurati e attribuiti
...
> 22 s** contro i **3 min 57 s** degli altri 984 messi insieme. Per il giro
...
portano — delimita i piazzamenti e non l'anagrafica, e la `School` non esiste. Ciò che mancava era che qualcuno guardasse: i 77 siti di
query della mattina erano **116** la sera, e ora
`tests/test_confine_orm.py` porta una **regola** (nucleo a zero) e un
**cricchetto** sui quarantuno punti dichiarati. Le query dentro i cicli sono
salite al caricamento — `check_schedule` **718 → 60**, `analyze_capacity()`
**1206 → 20**, stessi 344 finding. ⛔ **D2 e D4** sono sciolte il 2026-08-31 con
